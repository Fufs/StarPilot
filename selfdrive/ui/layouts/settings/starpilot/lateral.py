  ...
  def _build_view(self):
    ...
    sections = [
      ...
      SettingSection(tr_noop("Steering Modes"), [
        ...
        SettingRow("AlwaysOnLateralPauseOnBrakeSpeed", "value", tr_noop("Pause on Brake Press Below"),
                   subtitle="",
                   get_value=lambda: f"{self._params.get_int('AlwaysOnLateralPauseOnBrakeSpeed')} {"km/h" if self.get_bool("IsMetric") else "mph"}",
                   on_click=lambda: self._show_slider("AlwaysOnLateralPauseOnBrakeSpeed", 0, 100, unit=" km/h" if self.get_bool("IsMetric") else " mph"),
